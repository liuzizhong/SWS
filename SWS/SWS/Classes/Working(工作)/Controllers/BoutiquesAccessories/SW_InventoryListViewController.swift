//
//  SW_InventoryListViewController.swift
//  SWS
//
...
class SW_InventoryListViewController: SW_TableViewController {
    
    private var type: ProcurementType = .boutiques
    
    private var SW_InventoryListCellID = "SW_InventoryListCellID"
    
    ///  盘点单数据
    private var inventoryOrder = [SW_InventoryListModel]()
    
    private var searchDatas = [SW_InventoryListModel]()
    ...
    lazy var searchTableView: UITableView = {
        ...
        tbv.registerNib(SW_InventoryListCell.self, forCellReuseIdentifier: "SW_InventoryListCellID")
        tbv.rowHeight = 114
        return tbv
    }()
...
//        }
    ...
    private func setup() {
        ...
        tableView.registerNib(SW_InventoryListCell.self, forCellReuseIdentifier: SW_InventoryListCellID)
...
//            make.top.equalTo(searchBar.snp.bottom)
    ...
    func requestOrderList(_ keyword: String = "", isSearch: Bool = false, byAppend: Bool = false) {
        //        let offSet = byAppend ?  (isSearch ? searchDatas.count : inventoryOrder.count) : 0
        
        SW_WorkingService.getInventoryOrderList(type, keyWord: keyword).response({ (json, isCache, error) in
            self.emptyView.contentViewOffset = -(self.tableView.height - 250) * 0.1
            self.tableView.ly_emptyView = self.emptyView
            if let json = json as? JSON, error == nil {
                //                let totalCount = json["count"].intValue
                if byAppend {
                    let datas = json["list"].arrayValue.map({ (value) -> SW_InventoryListModel in
                        return SW_InventoryListModel(value, type: self.type)
                    })
                    if isSearch {
                        ...
                        self.inventoryOrder.append(contentsOf: datas)
...
//        stop()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == searchTableView {
            ...
            guard  inventoryOrder.count > indexPath.row else { return UITableViewCell() }
            model = inventoryOrder[indexPath.row]
        }
        let cell = tableView.dequeueReusableCell(withIdentifier: SW_InventoryListCellID, for: indexPath) as! SW_InventoryListCell
        cell.model = model
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == searchTableView {
            ...
            guard  inventoryOrder.count > indexPath.row else { return }
            model = inventoryOrder[indexPath.row]
        }
        navigationController?.pushViewController(SW_InventoryDetailViewController(model.id, type: type), animated: true)
    }
