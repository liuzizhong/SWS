//
//  SW_ProcurementListViewController.swift
//  SWS
//
...
class SW_ProcurementListViewController: SW_TableViewController {
    
    private var type: ProcurementType = .boutiques
    
    private var SW_ProcurementListCellID = "SW_ProcurementListCellID"
    
    /// 采购单数据
    private var purchaseOrder = [SW_ProcurementListModel]()
    
    private var searchDatas = [SW_ProcurementListModel]()
    ...
    lazy var searchTableView: UITableView = {
        ...
        tbv.registerNib(SW_ProcurementListCell.self, forCellReuseIdentifier: "SW_ProcurementListCellID")
//        tbv.rowHeight = UITableView.automaticDimension
        tbv.estimatedRowHeight = 94
        return tbv
    }()
...
//        }
    ...
    private func setup() {
        ...
        tableView.registerNib(SW_ProcurementListCell.self, forCellReuseIdentifier: SW_ProcurementListCellID)
...
//        let offSet = byAppend ?  (isSearch ? searchDatas.count : purchaseOrder.count) : 0
        
        var request: SWSRequest
        if type == .boutiques {
            request = SW_WorkingService.getBoutiqueInOrderList(keyword)
        } else {
            request = SW_WorkingService.getAccessoriesInOrderList(keyword)
        }
        ...
        request.response({ (json, isCache, error) in
            ...
            if let json = json as? JSON, error == nil {
                ...
                if byAppend {
                    let datas = json["list"].arrayValue.map({ (value) -> SW_ProcurementListModel in
                        return SW_ProcurementListModel(value, type: self.type)
                    })
                    if isSearch {
                        ...
                        self.purchaseOrder.append(contentsOf: datas)
...
//        stop()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var model: SW_ProcurementListModel
        if tableView == searchTableView {
            guard  searchDatas.count > indexPath.row else { return UITableViewCell() }
            model = searchDatas[indexPath.row]
        } else {
            guard  purchaseOrder.count > indexPath.row else { return UITableViewCell() }
            model = purchaseOrder[indexPath.row]
        }
        let cell = tableView.dequeueReusableCell(withIdentifier: SW_ProcurementListCellID, for: indexPath) as! SW_ProcurementListCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == searchTableView {
            ...
            guard  purchaseOrder.count > indexPath.row else { return }
            model = purchaseOrder[indexPath.row]
        }
        navigationController?.pushViewController(SW_ProcurementDetailViewController(model.purchaseOrderId, type: type), animated: true)
    }
