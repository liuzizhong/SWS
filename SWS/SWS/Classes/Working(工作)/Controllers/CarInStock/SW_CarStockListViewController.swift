//
//  SW_CarStockListViewController.swift
//  SWS
//
...
class SW_CarStockListViewController: SW_TableViewController {
    
    private var SW_CarInStockListCellID = "SW_CarInStockListCellID"
    
    /// 现在出来的数据 搜索的直接放这里面
    private var carStocks = [SW_CarInStockListModel]()
    
    private var searchDatas = [SW_CarInStockListModel]()
    ...
    lazy var searchTableView: UITableView = {
        ...
        tbv.registerNib(SW_CarInStockListCell.self, forCellReuseIdentifier: SW_CarInStockListCellID)
//        tbv.rowHeight = 126//UITableView.automaticDimension
        tbv.estimatedRowHeight = 136//type.rowHeight
        return tbv
    }()
    ...
    private lazy var searchEmptyView: LYEmptyView = {
        ...
        lazy var footerView: MJRefreshAutoNormalFooter = {
            let footer = MJRefreshAutoNormalFooter.init { [weak self] in
                self?.requestCarStockList(byAppend: true)
            }
    //        footer?.isAutomaticallyHidden = false
        ...
        lazy var searchFooterView: MJRefreshAutoNormalFooter = {
            let searchFooter = MJRefreshAutoNormalFooter.init { [weak self] in
                ...
                self.requestCarStockList(self.searchBar.searchText, isSearch: true, byAppend: true)
            }
    ...
    lazy var filterView: SW_CarInStockFilterHeaderView = {
        let view = Bundle.main.loadNibNamed("SW_CarInStockFilterHeaderView", owner: nil, options: nil)?.first as! SW_CarInStockFilterHeaderView
        return view
    }()
...
//        })
    ...
    private func setup() {
        ...
        tableView.registerNib(SW_CarInStockListCell.self, forCellReuseIdentifier: SW_CarInStockListCellID)
//        tableView.rowHeight = 126//UITableView.automaticDimension
        tableView.estimatedRowHeight = 136//type.rowHeight
        ...
        searchBar.searchBlock = { [weak self] in
            ...
            self.requestCarStockList(self.searchBar.searchText, isSearch: true)
        }
        
        filterView.selectValueChange = { [weak self] (type) in
            self?.selectType = type
            self?.requestCarStockList()
        }
    ...
    func requestCarStockList(_ keyword: String = "", isSearch: Bool = false, byAppend: Bool = false) {
        let offSet = byAppend ?  (isSearch ? searchDatas.count : carStocks.count) : 0
        
        SW_WorkingService.getCarStockList(keyword, type: selectType, max: 20, offset: offSet).response({ (json, isCache, error) in
            self.emptyView.contentViewOffset = -(self.tableView.height - 250) * 0.1
            self.tableView.ly_emptyView = self.emptyView
            if let json = json as? JSON, error == nil {
                let totalCount = json["count"].intValue
                if byAppend {
                    let datas = json["list"].arrayValue.map({ (value) -> SW_CarInStockListModel in
                        return SW_CarInStockListModel(value)
                    })
                    if isSearch {
                        ...
                        self.carStocks.append(contentsOf: datas)
                        if self.carStocks.count >= totalCount {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == searchTableView {
            ...
            guard  carStocks.count > indexPath.row else { return UITableViewCell() }
            model = carStocks[indexPath.row]
        }
        
        let cell = tableView.dequeueReusableCell(withIdentifier: SW_CarInStockListCellID, for: indexPath) as! SW_CarInStockListCell
        cell.model = model
        return cell
    }
