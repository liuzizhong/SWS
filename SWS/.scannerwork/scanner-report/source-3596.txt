...
@property (nonatomic, strong) UIImageView* resultPlateImg;
//@property (nonatomic, assign) pr::PipelinePR pr2;
...
static pr::PipelinePR sharePr2;

-(NSMutableDictionary *)resultDict
{
    if (_resultDict == nil) {
        _resultDict = [NSMutableDictionary dictionary];
        _resultDict[@"confidence"] = @(0);
        _resultDict[@"plateName"] = @"";
        _resultDict[@"plateImage"] = [[UIImage alloc] init];
    }
    return _resultDict;
}
...
- (void)initialize {
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharePr2 = *new pr::PipelinePR(*cpath_1, *cpath_2, *cpath_3, *cpath_4, *cpath_5, *cpath_6, *cpath_7, *cpath_8, *cpath_9);
    }) ;
...
- (void)createCameraView
{
    ...
    CGRect smallFrame = CGRectMake(30, 100, kScreenWidth - 60, (kScreenWidth - 60)*0.6);
    ...
    UIButton *backBtn = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth/16,20+top, backWidth, backWidth)];
    [backBtn addTarget:self action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];
    [backBtn setImage:[UIImage imageNamed:@"back_camera_btn"] forState:UIControlStateNormal];
    backBtn.titleLabel.textAlignment = NSTextAlignmentLeft;
    [self.view addSubview:backBtn];
    
    UIButton *flashBtn = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth-kScreenWidth/16-backWidth,20+top, backWidth, backWidth)];
...
//    [self.view bringSubviewToFront:photoBtn];
    ...
    _resultLb.frame = CGRectMake(80,60, 250, 30);
//    _resultLb.transform = CGAffineTransformMakeRotation(M_PI/2);
    _resultLb.font = [UIFont systemFontOfSize:16];
    [self.view addSubview:_resultLb];
    
    _resultPlateImg = [[UIImageView alloc] init];
//    _resultPlateImg.frame = CGRectMake(80, 400, 150, 200);
//    _resultPlateImg.translatesAutoresizingMaskIntoConstraints
    [self.view addSubview:_resultPlateImg];
    [_resultPlateImg mas_makeConstraints:^(MASConstraintMaker* make) {
...
{
    std::vector<pr::PlateInfo> list_res = sharePr2.RunPiplineAsImage(src,pr::SEGMENTATION_FREE_METHOD);
    NSString* concat_results = @"";
    
    for(auto one:list_res) {
        if(one.confidence>_confidence) {
            UIImage *plateImg;
            float confidence;
            
            float resultConfidence = [self.resultDict[@"confidence"] floatValue];
            if (resultConfidence > one.confidence) {
                concat_results = (NSString *)self.resultDict[@"plateName"];
                confidence = resultConfidence;
                plateImg = (UIImage *)self.resultDict[@"plateImage"];
            } else {
                concat_results = [NSString stringWithCString:one.getPlateName().c_str() encoding:NSUTF8StringEncoding];
                confidence = one.confidence;
                plateImg = [Utility UIImageFromCVMat:one.getPlateImage()];
            }
            
            dispatch_async(dispatch_get_main_queue(), ^{
                // UI更新代码
//                self.alert.text = @"Thanks!";
                self.resultLb.text = [NSString stringWithFormat:@"%@----%f", concat_results,confidence];
                self.resultPlateImg.image = plateImg;
            });
//            NSLog(@"===> confidence = %@", [NSString stringWithFormat:@"%f",one.confidence]);
            _confidence = 0.995;
            ...
            /// 识别出了车牌，但是自信度达不到要求，因此不达标中保存一个自信度最高的值，在成功后如果比那次成功的高，则使用自信度高的结果
            float resultConfidence = [self.resultDict[@"confidence"] floatValue];
            if (one.confidence > resultConfidence) {
                self.resultDict[@"confidence"] = @(one.confidence);
                self.resultDict[@"plateName"] = [NSString stringWithCString:one.getPlateName().c_str() encoding:NSUTF8StringEncoding];
                self.resultDict[@"plateImage"] = [Utility UIImageFromCVMat:one.getPlateImage()];
            }
...
//    if (concat_results.length > 0) {
//        concat_results = [concat_results substringToIndex:str.length-1];
//        concat_results = [NSString stringWithFormat:@"%@",concat_results];
//    } else {
//        concat_results = [NSString stringWithFormat:@"未识别成功"];
//    }
//    NSLog(@"===> 识别结果 = %@", str);
