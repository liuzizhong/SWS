//
//  SW_ContractPurchaseTaxEditViewController.swift
//  SWS
//
...
class SW_ContractPurchaseTaxEditViewController: FormViewController {
    ...
    private func createTableView() {
        form = Form()
            ...
            <<< SW_CommenFieldRow("purchaseTax") {
                $0.rawTitle = NSLocalizedString("车辆购置税", comment: "")
                $0.decimalCount = 4
                $0.amountMax = 9999999999.9999
                $0.isAmount = true
                $0.value = salesContract.carPurchaseTaxAmount == 0 ? "" : "\(salesContract.carPurchaseTaxAmount)"
                $0.cell.valueField.keyboardType = .decimalPad
                $0.cell.valueField.placeholder = "请输入车辆购置税"
                }.onChange { [weak self] in
                    self?.salesContract.carPurchaseTaxAmount = Double($0.value ?? "") ?? 0
        }
...
//
    ...
    @objc private func handleAction(_ sender: UIBarButtonItem) {
        ...
        if salesContract.carPurchaseTaxAmount == 0 {
            (form.rowBy(tag: "purchaseTax") as? SW_CommenFieldRow)?.showErrorLine()
            showAlertMessage("请输入车辆购置税", MYWINDOW)
            return
        }
    ...
    private func postRequest() {
        ...
        SW_SalesContractService.saveCarPurchaseTax(contractId: contractId, contract: salesContract).response({ (json, isCache, error) in
