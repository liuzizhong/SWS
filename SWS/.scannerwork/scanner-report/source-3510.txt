//
//  SW_LineChartViewCell.swift
//  SWS
//
//  Created by jayway on 2018/7/31.
...
class SW_LineChartViewCell: UITableViewCell {
...
//    }()
    ...
    var chartModel: SW_LineChartModel? {
        didSet {
...
//        chartView.highlightPerTapEnabled = false
    ...
    func updateChartData(_ chartModel: SW_LineChartModel) {
        //取出图表中线的个数 最少要1条  //x轴坐标个数最少1个
        guard chartModel.data.count > 0,
            let dataSetCount = chartModel.data.first?.value.count,
            dataSetCount > 0  else {
        ...
        legendLabels = chartModel.data[0].value.map({ return $0.name })
        colors = []//先清空颜色
        ...
        for i in 0..<dataSetCount {
            ...
            let yVals = (0..<chartModel.data.count).map { (x) -> ChartDataEntry in
                ...
                if chartModel.data[x].value.count > i {
                    y = Double(chartModel.data[x].value[i].getAmount(accountsType: chartModel.accountsType))
                } else {
                    y = 0
                }
...
extension SW_LineChartViewCell: UICollectionViewDelegate, UICollectionViewDataSource {
...
extension SW_LineChartViewCell: FixedSpacingCollectionLayoutDelegate {
...
extension SW_LineChartViewCell: ChartViewDelegate {
