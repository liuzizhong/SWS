//
//  SW_AfterSaleAddAccessRecordViewController.swift
//  SWS
//
...
class SW_AfterSaleAddAccessRecordViewController: FormViewController {
    
    private var record = SW_AfterSaleAccessRecordListModel()
    
    private var isRequesting = false
    
    init(_ customerId: String, repairOrderId: String) {
        super.init(nibName: nil, bundle: nil)
        self.record.customerId = customerId
        self.record.repairOrderId = repairOrderId
    }
    ...
    private func formConfig() {
        ...
        navigationItem.title = NSLocalizedString("添加接访记录", comment: "")
    ...
    private func createTableView() {
        ...
        form = Form()
            ...
            <<< SW_SingleChoiceRow("Access type") {
                ...
                $0.allOptions = ["电话访问","维修接待"]
                }.onChange { [weak self] in
                    self?.record.accessType = AfterSaleAccessType($0.value ?? "")
            }
    ...
    private func postRequest() {
        ...
        SW_AfterSaleService.saveAccessCustomerRecord(record).response({ (json, isCache, error) in
...
extension SW_AfterSaleAddAccessRecordViewController {
